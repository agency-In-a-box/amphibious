...
async function createBackup(filePath: string): Promise<void> {
  const backupDir = join(process.cwd(), 'backups', 'memory-leaks', new Date().toISOString().split('T')[0]);
...
async function fixModalMemoryLeaks(): Promise<MemoryLeakFix> {
  ...
  content = content.slice(0, firstProperty) +
...
` + content.slice(initMethod);
  ...
  const setupEventHandlersEnd = content.indexOf('  }', content.indexOf('    }', setupEventHandlersStart)) + 3;
  ...
  content = content.slice(0, setupEventHandlersStart) + newSetupEventHandlers + content.slice(setupEventHandlersEnd);

  // Fix createBackdrop method
  const createBackdropMethod = content.indexOf('  private createBackdrop(): void {');
  const createBackdropEnd = content.indexOf('  }', content.indexOf('    }', createBackdropMethod)) + 3;
  ...
  content = content.slice(0, createBackdropMethod) + newCreateBackdrop + content.slice(createBackdropEnd);

  // Fix destroy method - remove cloneNode hack
  const destroyMethod = content.indexOf('  public destroy(): void {');
  const destroyEnd = content.indexOf('  }', content.indexOf('    this.element.removeAttribute(\'tabindex\');', destroyMethod)) + 3;
