...
(($) => {
  function Sequence(element, options, defaults, get) {
    var self = this;
    (self.container = $(element)), (self.sequence = self.container.children('ul'));
    ...
    var prefixes = {
      ...
      transitions = {
        //convert JS transition names to JS transition end and animation end event names
    ...
    (self.prefix =
      ...
      (self.transitionEnd =
        transitions[
          Modernizr.prefixed('transition')
        ]), //work out the JS transitionEnd name for the browser being used (webkitTransitionEnd webkitAnimationEnd for example)
      (self.transitionProperties = {}),
      (self.numberOfFrames =
        self.sequence.children(
          'li',
        ).length), //number of frames (<li>) Sequence consists of
      ...
      (self.hasTouch =
        'ontouchstart' in window
          ? true
          : false), //determine if this is a touch enabled device
      ...
      (self.init = {
        /*functionality to initiate the preloader, next/previous buttons and so on
			...
			*/
        uiElements: (devOption, defaultOption) => {
          switch (devOption) {
    ...
    (self.paused =
      ...
      (self.afterLoaded = () => {}); //executes after Sequence is initiated
    ...
    function oncePreloaded() {
      ...
      if (self.settings.preloader) {
        if (self.settings.hidePreloaderUsingCSS && self.transitionsSupported) {
          ...
          self.settings.preloader.fadeOut(self.settings.hidePreloaderDelay, () => {
      ...
      function saveImagesToArray(length, srcOnly) {
        ...
        if (!srcOnly) {
          for (var i = length; i > 0; i--) {
            ...
            self.sequence
              .children('li:nth-child(' + self.settings.preloadTheseFrames[i - 1] + ')')
              .find('img')
              .each(function () {
          ...
          for (var i = length; i > 0; i--) {
            ...
            imagesToPreload.push(
              $('body').find('img[src="' + self.settings.preloadTheseImages[i - 1] + '"]')[0],
    ...
    function imagesLoaded(imagesToPreload, callback) {
      ...
      if (!$images.length) {
        ...
        $images
          .bind('load.imagesLoaded error.imagesLoaded', (event) => {
      ...
      if (self.settings.keyNavigation) {
        ...
        function keyEvents(keyPressed, keyDirections) {
          ...
          for (keyCodes in keyDirections) {
            ...
            if (keyPressed === Number.parseFloat(keyCode)) {
  ...
  Sequence.prototype = {
    ...
    reverseTransitionProperties: function () {
      ...
      self.frameChildren.each(function () {
        ...
        currentFrameChildrenDurations.push(
          Number.parseFloat(
            $(this)
              .css(self.prefix + 'transition-duration')
              .replace('s', ''),
          ) +
            Number.parseFloat(
              $(this)
                .css(self.prefix + 'transition-delay')
                .replace('s', ''),
            ),
        );
      });
      ...
      if (transitionDifference === 0) {
        //if the duration difference is the same, neither frame need be given a delay
        currentDelay = 0;
        nextDelay = 0;
      ...
      function reverseEachProperty(frameChildren, maximumFrameDuration, frameDelay) {
        frameChildren.each(function () {
          ...
          if (transitionFunction.indexOf('cubic-bezier') >= 0) {
            ...
            $.each(cubicBezier, (index, value) => {
              //for each point that makes up the cubic bezier...
              cubicBezier[index] = Number.parseFloat(value); //turn the point into a number (rather than text)
            });
          ...
          self.transitionProperties['transition-delay'] =
    ...
    startAutoPlay: function (delay) {
      var delay = delay === undefined ? this.settings.autoPlayDelay : delay; //if a delay isn't specified, use the default
      this.unpause();

      this.resetAutoPlay(); //stop autoPlay before starting it again
      this.autoPlayTimer = setTimeout(() => {
        //start a new autoPlay timer and...
        this.settings.autoPlayDirection === 1 ? this.next() : this.prev(); //go to either the next or previous frame
    ...
    stopAutoPlay: function () {
      this.pause(true);
      clearTimeout(this.autoPlayTimer); //stop the autoPlay timer
    },
    ...
    pause: function (hardPause) {
      if (!this.isPaused) {
        ...
        if (this.pauseButton !== undefined) {
          //if a pause button is defined...
          this.pauseButton.addClass('paused'); //add the class of "paused" to the pause button
          if (this.pauseIcon !== undefined) {
            //if a pause icon is defined...
            this.pauseIcon.show(); //show the pause icon
          }
        }
        this.paused(); //callback when Sequence is paused
        this.isPaused = true;
        this.isHardPaused = hardPause ? true : false; //if hardPausing, set hardPause to true
        this.resetAutoPlay(); //stop autoPlay
      } else {
    ...
    unpause: function (callback) {
      if (this.pauseButton !== undefined) {
        //if a pause button is defined...
        this.pauseButton.removeClass('paused'); //remove the class of "paused" from the pause button
        if (this.pauseIcon !== undefined) {
          //if a pause icon is defined...
          this.pauseIcon.hide(); //hide the pause icon
        }
      }
      ...
      if (!this.active) {
        if (callback !== false) {
          this.unpaused(); //callback when Sequence is unpaused
        }
        this.resetAutoPlay(true, this.settings.unpauseDelay); //start autoPlay after a delay specified via the unpauseDelay setting
      } else {
        this.delayUnpause = true; //Sequence is animating so delay the unpause event until the animation completes
      }
    },

    //Go to the frame ahead of the current one
    next: function () {
      this.nextFrameID = this.currentFrameID !== this.numberOfFrames ? this.currentFrameID + 1 : 1; //work out the next frame
      this.goTo(this.nextFrameID, 1); //go to the next frame
    },

    //Go to the frame prior to the current one
    prev: function () {
      this.nextFrameID = this.currentFrameID === 1 ? this.numberOfFrames : this.currentFrameID - 1; //work out the prev frame
      this.goTo(this.nextFrameID, -1); //go to the prev frame
    },
    ...
    goTo: function (id, direction, ignoreTransitionThreshold) {
      var self = this;
      var id = Number.parseFloat(id); //convert the id to a number just in case
      var transitionThreshold =
        ignoreTransitionThreshold === true ? 0 : self.settings.transitionThreshold; //if transitionThreshold is to be ignored, set it to zero
        ...
        if (self.transitionsSupported) {
          ...
          if (!self.settings.reverseAnimationsWhenNavigatingBackwards || self.direction === 1) {
            ...
            setTimeout(() => {
              ...
              if (
                ...
                self.transitionThresholdTimer = setTimeout(() => {
          ...
          switch (self.settings.fallback.theme) {
            case 'fade': //if using the fade fallback theme...
              ...
              self.currentFrame.animate({ opacity: 0 }, self.settings.fallback.speed, () => {
            ...
            default:
              ...
              self.beforeCurrentFrameAnimatesOut();
              self.currentFrame.animate(animateOut, self.settings.fallback.speed, () => {
                self.afterCurrentFrameAnimatesOut();
              }); //cause the current frame to animate out
              self.beforeNextFrameAnimatesIn(); //callback
              self.nextFrame.show().css(animateIn);
              self.nextFrame.animate(moveIn, self.settings.fallback.speed, () => {
    ...
    /*
		...
		*/
    waitForAnimationsToComplete: function (
      frame,
      frameChildren,
      transitionPhase,
      inAfterwards,
      ...
      if (transitionPhase === 'out') {
        ...
        var onceComplete = () => {
          this.afterCurrentFrameAnimatesOut(); //callback

          if (this.settings.transitionThreshold === true) {
            if (direction === 1) {
              this.nextFrame.addClass('animate-in'); //add the "animate-in" class
            } else if (direction === -1) {
              this.nextFrame.toggleClass('animate-out animate-in');
            }
          }
        };
        ...
        var onceComplete = () => {
          this.afterNextFrameAnimatesIn(); //callback
          this.setHashTag(); //set the hashtag to represent the newly active frame

          this.active = false; //Sequence is not animating

          if (!this.isHardPaused && !this.mouseover) {
            //if Sequence isn't hard paused (via a pause button for example) or being hovered over...
            if (!this.delayUnpause) {
              //if unpausing isn't delayed (Sequence wasn't animating when unpause was invoked)...
              this.unpause(false); //unpause Sequence but don't run the unpause callback
            } else {
              //if unpausing was delay because Sequence was animating when unpause was invoked...
              this.delayUnpause = false;
              this.unpause(); //unpause Sequence
            }
          }
        };
      }
      ...
      frame.bind(this.transitionEnd, (e) => {
        ...
        if (allAnimationsEnded) {
          ...
          frame.unbind(this.transitionEnd); //stop waiting for animations to end
          onceComplete();
        }
      });
    },
    ...
    setHashTag: function () {
      if (this.settings.hashTags) {
        ...
        this.currentHashTag = this.nextFrame.attr(this.getHashTagFrom); //get the hashtag name
        this.frameHashIndex = $.inArray(this.currentHashTag, this.frameHashID); //get the index of the frame that matches the hashtag
        if (
    ...
    modernizr: () => {
      window.Modernizr = ((a, b, c) => {
        ...
        !z(v, 'undefined') && !z(v.call, 'undefined')
          ? (w = (a, b) => v.call(a, b))
          : (w = (a, b) => b in a && z(a.constructor.prototype[b], 'undefined')),
          ...
          (p.svg = () => !!b.createElementNS && !!b.createElementNS(o.svg, 'svg').createSVGRect);
        for (var E in p)
        ...
        return (
          ...
          (e.testProp = (a) => B([a])),
          (e.testAllProps = D),
          (e.prefixed = (a, b, c) => (b ? D(a, b, c) : D(a, 'pfx'))),
          e
        );
      })(self, self.document);
    },

    defaultPreloader: (prependTo, transitions, prefix) => {
      var icon =
