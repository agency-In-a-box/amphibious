/**
 * Enhanced Date Picker Component TypeScript
 * Comprehensive date selection with full cleanup and advanced features
 * Part of Amphibious 2.0 Component Library
 *
 * @module datepicker-enhanced
 */

/**
 * Localizable UI labels for the enhanced datepicker.
 *
 * @property datePicker - ARIA label for the calendar dialog.
 * @property previousMonth - ARIA label for the previous navigation button.
 * @property nextMonth - ARIA label for the next navigation button.
 * @property today - Label for the "Today" shortcut/button.
 * @property clear - Label for the "Clear" button.
 * @property yesterday - Label for the "Yesterday" shortcut.
 * @property lastWeek - Label for the "Last Week" shortcut.
 * @property lastMonth - Label for the "Last Month" shortcut.
 */
export interface DatePickerEnhancedLabels {
  datePicker: string;
  previousMonth: string;
  nextMonth: string;
  today: string;
  clear: string;
  yesterday: string;
  lastWeek: string;
  lastMonth: string;
}

/**
 * Callback fired when a specific date is selected.
 */
type OnSelectCallback = (date: Date, picker: DatePickerEnhanced) => void;

/**
 * Callback fired when the selected value changes (including clear).
 */
type OnChangeCallback = (
  value: Date | Date[] | DatePickerEnhancedRangeValue | null,
  picker: DatePickerEnhanced,
) => void;

/**
 * Callback fired when the calendar opens or closes.
 */
type OnLifecycleCallback = (picker: DatePickerEnhanced) => void;

/** Calendar view mode for navigating between day, month, and year grids. */
type ViewMode = 'days' | 'months' | 'years';

/** Calendar positioning strategy. */
type PositionMode = 'auto' | 'top' | 'bottom';

/**
 * Value returned by {@link DatePickerEnhanced.getValue} when in range mode.
 */
export interface DatePickerEnhancedRangeValue {
  start: Date | null;
  end: Date | null;
}

/**
 * Value accepted by {@link DatePickerEnhanced.setValue} when in range mode.
 */
interface SetRangeValue {
  start: Date | string;
  end: Date | string;
}

/**
 * Configuration options accepted by the {@link DatePickerEnhanced} constructor.
 * All properties are optional; sensible defaults are applied internally.
 *
 * @property format - Date format string (e.g. `'MM/DD/YYYY'`).
 * @property minDate - Earliest selectable date.
 * @property maxDate - Latest selectable date.
 * @property disabledDates - Array of specific dates that cannot be selected.
 * @property disabledDays - Array of weekday indices (0=Sunday, 6=Saturday) to disable.
 * @property startDate - Initial date shown by the calendar.
 * @property inline - Render the calendar inline instead of as a dropdown.
 * @property showTime - Include time-picker controls.
 * @property autoClose - Close after a date is selected. Defaults to `true`.
 * @property onChange - Callback fired when the selected value changes.
 * @property onOpen - Callback fired when the calendar opens.
 * @property onClose - Callback fired when the calendar closes.
 * @property onSelect - Callback fired when a specific date is selected.
 * @property weekStart - First day of the week (0=Sunday, 1=Monday). Defaults to `0`.
 * @property yearRange - Number of years to show in the year picker. Defaults to `100`.
 * @property showWeekNumbers - Show ISO week numbers beside rows.
 * @property allowClear - Allow clearing the selection. Defaults to `true`.
 * @property multiple - Enable multi-date selection.
 * @property range - Enable date range selection.
 * @property shortcuts - Show date shortcut buttons. Defaults to `true`.
 * @property position - Calendar dropdown position. Defaults to `'auto'`.
 * @property theme - Visual theme name. Defaults to `'light'`.
 * @property locale - BCP 47 locale tag. Defaults to `'en-US'`.
 * @property labels - Localizable UI label strings.
 * @property animations - Enable open/close animations. Defaults to `true`.
 */
export interface DatePickerEnhancedOptions {
  format?: string;
  minDate?: Date | null;
  maxDate?: Date | null;
  disabledDates?: (Date | string)[];
  disabledDays?: number[];
  startDate?: Date;
  inline?: boolean;
  showTime?: boolean;
  autoClose?: boolean;
  onChange?: OnChangeCallback | null;
  onOpen?: OnLifecycleCallback | null;
  onClose?: OnLifecycleCallback | null;
  onSelect?: OnSelectCallback | null;
  weekStart?: number;
  yearRange?: number;
  showWeekNumbers?: boolean;
  allowClear?: boolean;
  multiple?: boolean;
  range?: boolean;
  shortcuts?: boolean;
  position?: PositionMode;
  theme?: string;
  locale?: string;
  labels?: Partial<DatePickerEnhancedLabels>;
  animations?: boolean;
}

/** Resolved internal options with all defaults applied. */
interface ResolvedOptions {
  format: string;
  minDate: Date | null;
  maxDate: Date | null;
  disabledDates: (Date | string)[];
  disabledDays: number[];
  startDate: Date;
  inline: boolean;
  showTime: boolean;
  autoClose: boolean;
  onChange: OnChangeCallback | null;
  onOpen: OnLifecycleCallback | null;
  onClose: OnLifecycleCallback | null;
  onSelect: OnSelectCallback | null;
  weekStart: number;
  yearRange: number;
  showWeekNumbers: boolean;
  allowClear: boolean;
  multiple: boolean;
  range: boolean;
  shortcuts: boolean;
  position: PositionMode;
  theme: string;
  locale: string;
  labels: DatePickerEnhancedLabels;
  animations: boolean;
}

/**
 * Internal state for the enhanced datepicker.
 */
interface DatePickerState {
  isOpen: boolean;
  viewMode: ViewMode;
  viewDate: Date;
  selectedDates: Date[];
  rangeStart: Date | null;
  rangeEnd: Date | null;
  hoveredDate: Date | null;
  focusedDate: Date | null;
}

/**
 * Stored event handler entry for cleanup tracking.
 */
interface HandlerEntry {
  event: string;
  handler: EventListener;
  options?: AddEventListenerOptions;
}

/**
 * Stored dynamic (render-cycle) event handler entry.
 */
interface DynamicHandlerEntry {
  event: string;
  handler: EventListener;
}

/**
 * Shortcut button descriptor used in the shortcuts panel.
 */
interface ShortcutDescriptor {
  label: string;
  value: () => Date;
}

/**
 * Enhanced date picker with multi-select, range selection, shortcuts,
 * keyboard navigation, i18n, and comprehensive cleanup.
 *
 * Wraps an `<input>` element with a dropdown calendar UI. All event
 * listeners are tracked for deterministic teardown via {@link destroy}.
 *
 * @example
 * ```ts
 * const picker = new DatePickerEnhanced(document.querySelector('#date')!, {
 *   range: true,
 *   locale: 'en-GB',
 *   onChange: (value) => console.log('Selected:', value),
 * });
 * ```
 */
class DatePickerEnhanced {
  public element!: HTMLInputElement | null;
  public options!: ResolvedOptions | null;
  public state!: DatePickerState | null;

  // Cleanup tracking collections
  private handlers: Map<EventTarget, HandlerEntry[]>;
  private dynamicHandlers: Map<Element, DynamicHandlerEntry>;
  private timers: Set<ReturnType<typeof setTimeout>>;
  private createdElements: Set<HTMLElement>;

  // DOM references set during init() → setupDOM() → createCalendar()
  public wrapper!: HTMLDivElement;
  public calendar!: HTMLDivElement;
  public body!: HTMLDivElement;
  public prevBtn!: HTMLButtonElement;
  public nextBtn!: HTMLButtonElement;
  public monthText!: HTMLButtonElement;
  public yearText!: HTMLButtonElement;
  public todayBtn!: HTMLButtonElement;
  public clearBtn!: HTMLButtonElement;

  constructor(element: HTMLInputElement, options: DatePickerEnhancedOptions = {}) {
    this.element = element;

    // Store all event handlers for proper cleanup
    this.handlers = new Map();
    this.dynamicHandlers = new Map();

    // Store all intervals/timeouts
    this.timers = new Set();

    // Store all created DOM elements
    this.createdElements = new Set();

    this.options = {
      format: options.format || 'MM/DD/YYYY',
      minDate: options.minDate || null,
      maxDate: options.maxDate || null,
      disabledDates: options.disabledDates || [],
      disabledDays: options.disabledDays || [], // 0=Sunday, 6=Saturday
      startDate: options.startDate || new Date(),
      inline: options.inline || false,
      showTime: options.showTime || false,
      autoClose: options.autoClose !== false,
      onChange: options.onChange || null,
      onOpen: options.onOpen || null,
      onClose: options.onClose || null,
      onSelect: options.onSelect || null,
      weekStart: options.weekStart || 0, // 0=Sunday, 1=Monday
      yearRange: options.yearRange || 100,
      showWeekNumbers: options.showWeekNumbers || false,
      allowClear: options.allowClear !== false,
      multiple: options.multiple || false,
      range: options.range || false,
      shortcuts: options.shortcuts !== false,
      position: options.position || 'auto', // auto, top, bottom
      theme: options.theme || 'light',
      locale: options.locale || 'en-US',
      labels: {
        datePicker: 'Date picker',
        previousMonth: 'Previous month',
        nextMonth: 'Next month',
        today: 'Today',
        clear: 'Clear',
        yesterday: 'Yesterday',
        lastWeek: 'Last Week',
        lastMonth: 'Last Month',
        ...(options.labels || {}),
      },
      animations: options.animations !== false,
    };

    // State management
    this.state = {
      isOpen: false,
      viewMode: 'days',
      viewDate: new Date(this.options.startDate),
      selectedDates: [],
      rangeStart: null,
      rangeEnd: null,
      hoveredDate: null,
      focusedDate: null,
    };

    // Parse existing value
    if (this.element.value) {
      this.parseInitialValue();
    }

    this.init();
  }

  private init(): void {
    this.setupDOM();
    this.bindEvents();
    this.setupKeyboardNav();

    if (this.options!.inline) {
      this.open();
    }
  }

  private setupDOM(): void {
    // Create wrapper
    const wrapper = document.createElement('div');
    wrapper.className = 'aiab-datepicker-wrapper';
    if (this.options!.theme) {
      wrapper.classList.add(`aiab-datepicker-theme-${this.options!.theme}`);
    }

    // Wrap the input
    this.element!.parentNode!.insertBefore(wrapper, this.element);
    wrapper.appendChild(this.element!);

    // Make input readonly if not inline
    if (!this.options!.inline) {
      this.element!.setAttribute('readonly', 'readonly');
    }

    // Add classes
    this.element!.classList.add('aiab-datepicker-input');

    // Store references
    this.wrapper = wrapper;
    this.createdElements.add(wrapper);

    // Create calendar container
    this.createCalendar();
  }

  private createCalendar(): void {
    const calendar = document.createElement('div');
    calendar.className = 'aiab-datepicker-calendar';
    calendar.setAttribute('role', 'dialog');
    calendar.setAttribute('aria-label', this.options!.labels.datePicker);
    calendar.setAttribute('aria-hidden', 'true');

    // Header
    const header = this.createHeader();
    calendar.appendChild(header);

    // Shortcuts
    if (this.options!.shortcuts) {
      const shortcuts = this.createShortcuts();
      calendar.appendChild(shortcuts);
    }

    // Body
    const body = document.createElement('div');
    body.className = 'aiab-datepicker-body';
    calendar.appendChild(body);

    // Footer
    const footer = this.createFooter();
    calendar.appendChild(footer);

    // Add to wrapper
    this.wrapper.appendChild(calendar);

    this.calendar = calendar as HTMLDivElement;
    this.body = body as HTMLDivElement;
    this.createdElements.add(calendar);

    // Initial render
    this.render();
  }

  private createHeader(): HTMLDivElement {
    const header = document.createElement('div');
    header.className = 'aiab-datepicker-header';

    // Previous button
    const prevBtn = document.createElement('button');
    prevBtn.className = 'aiab-datepicker-prev';
    prevBtn.type = 'button';
    prevBtn.innerHTML = '\u2039';
    prevBtn.setAttribute('aria-label', this.options!.labels.previousMonth);

    // Month/Year display
    const display = document.createElement('div');
    display.className = 'aiab-datepicker-display';

    const monthText = document.createElement('button');
    monthText.className = 'aiab-datepicker-month-text';
    monthText.type = 'button';

    const yearText = document.createElement('button');
    yearText.className = 'aiab-datepicker-year-text';
    yearText.type = 'button';

    display.appendChild(monthText);
    display.appendChild(yearText);

    // Next button
    const nextBtn = document.createElement('button');
    nextBtn.className = 'aiab-datepicker-next';
    nextBtn.type = 'button';
    nextBtn.innerHTML = '\u203A';
    nextBtn.setAttribute('aria-label', this.options!.labels.nextMonth);

    header.appendChild(prevBtn);
    header.appendChild(display);
    header.appendChild(nextBtn);

    // Store references
    this.prevBtn = prevBtn;
    this.nextBtn = nextBtn;
    this.monthText = monthText;
    this.yearText = yearText;

    this.createdElements.add(header);

    return header as HTMLDivElement;
  }

  private createShortcuts(): HTMLDivElement {
    const container = document.createElement('div');
    container.className = 'aiab-datepicker-shortcuts';

    const labels = this.options!.labels;
    const shortcuts: ShortcutDescriptor[] = [
      { label: labels.today, value: () => new Date() },
      {
        label: labels.yesterday,
        value: () => {
          const d = new Date();
          d.setDate(d.getDate() - 1);
          return d;
        },
      },
      {
        label: labels.lastWeek,
        value: () => {
          const d = new Date();
          d.setDate(d.getDate() - 7);
          return d;
        },
      },
      {
        label: labels.lastMonth,
        value: () => {
          const d = new Date();
          d.setMonth(d.getMonth() - 1);
          return d;
        },
      },
    ];

    for (const shortcut of shortcuts) {
      const btn = document.createElement('button');
      btn.className = 'aiab-datepicker-shortcut';
      btn.type = 'button';
      btn.textContent = shortcut.label;

      // Store handler
      const handler: EventListener = () => this.selectDate(shortcut.value());
      this.addHandler(btn, 'click', handler);

      container.appendChild(btn);
    }

    this.createdElements.add(container);
    return container as HTMLDivElement;
  }

  private createFooter(): HTMLDivElement {
    const footer = document.createElement('div');
    footer.className = 'aiab-datepicker-footer';

    // Today button
    const todayBtn = document.createElement('button');
    todayBtn.className = 'aiab-datepicker-today';
    todayBtn.type = 'button';
    todayBtn.textContent = this.options!.labels.today;

    // Clear button
    const clearBtn = document.createElement('button');
    clearBtn.className = 'aiab-datepicker-clear';
    clearBtn.type = 'button';
    clearBtn.textContent = this.options!.labels.clear;

    footer.appendChild(clearBtn);
    footer.appendChild(todayBtn);

    this.todayBtn = todayBtn;
    this.clearBtn = clearBtn;

    this.createdElements.add(footer);

    return footer as HTMLDivElement;
  }

  private bindEvents(): void {
    // Input events
    this.addHandler(this.element!, 'click', () => this.toggle());
    this.addHandler(this.element!, 'focus', () => this.open());
    this.addHandler(this.element!, 'blur', (_e: Event) => {
      // Close if focus moves outside aiab-datepicker
      const timer = setTimeout(() => {
        if (!this.wrapper.contains(document.activeElement)) {
          this.close();
        }
      }, 200);
      this.timers.add(timer);
    });

    // Navigation
    this.addHandler(this.prevBtn, 'click', () => this.navigate(-1));
    this.addHandler(this.nextBtn, 'click', () => this.navigate(1));

    // View mode switches
    this.addHandler(this.monthText, 'click', () => this.setViewMode('months'));
    this.addHandler(this.yearText, 'click', () => this.setViewMode('years'));

    // Footer buttons
    this.addHandler(this.todayBtn, 'click', () => this.selectToday());
    this.addHandler(this.clearBtn, 'click', () => this.clear());

    // Click outside to close
    const outsideHandler: EventListener = (e: Event) => {
      if (!this.wrapper.contains(e.target as Node) && this.state!.isOpen) {
        this.close();
      }
    };
    this.addHandler(document, 'click', outsideHandler);

    // Window resize
    const resizeHandler: EventListener = () => this.updatePosition();
    this.addHandler(window, 'resize', resizeHandler);

    // ESC key to close
    const escHandler: EventListener = (e: Event) => {
      const keyEvent = e as KeyboardEvent;
      if (keyEvent.key === 'Escape' && this.state!.isOpen) {
        this.close();
        this.element!.focus();
      }
    };
    this.addHandler(document, 'keydown', escHandler);
  }

  private setupKeyboardNav(): void {
    const keyHandler: EventListener = (e: Event) => {
      const keyEvent = e as KeyboardEvent;
      if (!this.state!.isOpen) return;

      const focused = this.state!.focusedDate || this.state!.viewDate;
      const newDate = new Date(focused);
      let preventDefault = false;

      switch (keyEvent.key) {
        case 'ArrowLeft':
          newDate.setDate(newDate.getDate() - 1);
          preventDefault = true;
          break;
        case 'ArrowRight':
          newDate.setDate(newDate.getDate() + 1);
          preventDefault = true;
          break;
        case 'ArrowUp':
          newDate.setDate(newDate.getDate() - 7);
          preventDefault = true;
          break;
        case 'ArrowDown':
          newDate.setDate(newDate.getDate() + 7);
          preventDefault = true;
          break;
        case 'PageUp':
          if (keyEvent.shiftKey) {
            newDate.setFullYear(newDate.getFullYear() - 1);
          } else {
            newDate.setMonth(newDate.getMonth() - 1);
          }
          preventDefault = true;
          break;
        case 'PageDown':
          if (keyEvent.shiftKey) {
            newDate.setFullYear(newDate.getFullYear() + 1);
          } else {
            newDate.setMonth(newDate.getMonth() + 1);
          }
          preventDefault = true;
          break;
        case 'Home':
          newDate.setDate(1);
          preventDefault = true;
          break;
        case 'End':
          newDate.setMonth(newDate.getMonth() + 1);
          newDate.setDate(0);
          preventDefault = true;
          break;
        case 'Enter':
        case ' ':
          this.selectDate(focused);
          preventDefault = true;
          break;
      }

      if (preventDefault) {
        keyEvent.preventDefault();
        this.state!.focusedDate = newDate;
        this.state!.viewDate = new Date(newDate);
        this.render();
      }
    };

    this.addHandler(this.calendar, 'keydown', keyHandler);
  }

  /**
   * Add an event listener with cleanup tracking.
   */
  private addHandler(
    element: EventTarget,
    event: string,
    handler: EventListener,
    options?: AddEventListenerOptions,
  ): void {
    element.addEventListener(event, handler, options);

    if (!this.handlers.has(element)) {
      this.handlers.set(element, []);
    }
    this.handlers.get(element)!.push({ event, handler, options });
  }

  // --- Render methods ---

  private render(): void {
    switch (this.state!.viewMode) {
      case 'days':
        this.renderDays();
        break;
      case 'months':
        this.renderMonths();
        break;
      case 'years':
        this.renderYears();
        break;
    }

    this.updateHeader();
  }

  private renderDays(): void {
    const year = this.state!.viewDate.getFullYear();
    const month = this.state!.viewDate.getMonth();
    const firstDay = new Date(year, month, 1).getDay();
    const daysInMonth = new Date(year, month + 1, 0).getDate();
    const daysInPrevMonth = new Date(year, month, 0).getDate();

    let html = '<div class="aiab-datepicker-days">';

    // Week headers
    html += '<div class="aiab-datepicker-weekdays">';
    const weekDays = this.getWeekDays();
    for (const day of weekDays) {
      html += `<div class="aiab-datepicker-weekday">${day}</div>`;
    }
    html += '</div>';

    // Days grid
    html += '<div class="aiab-datepicker-dates">';

    // Previous month days
    const startDay = (firstDay - this.options!.weekStart + 7) % 7;
    for (let i = startDay - 1; i >= 0; i--) {
      const day = daysInPrevMonth - i;
      html += `<button type="button" class="aiab-datepicker-date aiab-datepicker-date--other-month"
                data-date="${year}-${month}-${day}">${day}</button>`;
    }

    // Current month days
    for (let day = 1; day <= daysInMonth; day++) {
      const date = new Date(year, month, day);
      const classes = this.getDateClasses(date);
      const disabled = this.isDateDisabled(date);

      html += `<button type="button" class="aiab-datepicker-date ${classes}"
                data-date="${year}-${month + 1}-${day}"
                ${disabled ? 'disabled' : ''}>${day}</button>`;
    }

    // Next month days
    const totalCells = 42; // 6 weeks
    const filledCells = startDay + daysInMonth;
    for (let day = 1; day <= totalCells - filledCells; day++) {
      html += `<button type="button" class="aiab-datepicker-date aiab-datepicker-date--other-month"
                data-date="${year}-${month + 2}-${day}">${day}</button>`;
    }

    html += '</div></div>';

    this.body.innerHTML = html;

    // Bind date click events
    this.body.querySelectorAll<HTMLButtonElement>('.aiab-datepicker-date').forEach((btn) => {
      if (!btn.disabled) {
        const handler: EventListener = () => {
          const parts = btn.dataset.date!.split('-').map(Number);
          this.selectDate(new Date(parts[0], parts[1] - 1, parts[2]));
        };
        btn.addEventListener('click', handler);
        this.dynamicHandlers.set(btn, { event: 'click', handler });
      }
    });
  }

  private renderMonths(): void {
    const months = this.getMonthNames();
    const currentMonth = new Date().getMonth();
    const viewMonth = this.state!.viewDate.getMonth();

    let html = '<div class="aiab-datepicker-months">';

    months.forEach((month: string, index: number) => {
      const classes: string[] = [];
      if (index === currentMonth) classes.push('aiab-datepicker-month--current');
      if (index === viewMonth) classes.push('aiab-datepicker-month--selected');

      html += `<button type="button" class="aiab-datepicker-month ${classes.join(' ')}"
                data-month="${index}">${month}</button>`;
    });

    html += '</div>';

    this.body.innerHTML = html;

    // Bind month click events
    this.body.querySelectorAll<HTMLButtonElement>('.aiab-datepicker-month').forEach((btn) => {
      const handler: EventListener = () => {
        this.state!.viewDate.setMonth(Number.parseInt(btn.dataset.month!, 10));
        this.state!.viewMode = 'days';
        this.render();
      };
      btn.addEventListener('click', handler);
      this.dynamicHandlers.set(btn, { event: 'click', handler });
    });
  }

  private renderYears(): void {
    const currentYear = new Date().getFullYear();
    const viewYear = this.state!.viewDate.getFullYear();
    const startYear = Math.floor(viewYear / 10) * 10;

    let html = '<div class="aiab-datepicker-years">';

    for (let year = startYear - 1; year <= startYear + 10; year++) {
      const classes: string[] = [];
      if (year === currentYear) classes.push('aiab-datepicker-year--current');
      if (year === viewYear) classes.push('aiab-datepicker-year--selected');
      if (year === startYear - 1 || year === startYear + 10) {
        classes.push('aiab-datepicker-year--other');
      }

      html += `<button type="button" class="aiab-datepicker-year ${classes.join(' ')}"
                data-year="${year}">${year}</button>`;
    }

    html += '</div>';

    this.body.innerHTML = html;

    // Bind year click events
    this.body.querySelectorAll<HTMLButtonElement>('.aiab-datepicker-year').forEach((btn) => {
      const handler: EventListener = () => {
        this.state!.viewDate.setFullYear(Number.parseInt(btn.dataset.year!, 10));
        this.state!.viewMode = 'months';
        this.render();
      };
      btn.addEventListener('click', handler);
      this.dynamicHandlers.set(btn, { event: 'click', handler });
    });
  }

  // --- Date helper methods ---

  private getDateClasses(date: Date): string {
    const classes: string[] = [];
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    date.setHours(0, 0, 0, 0);

    if (date.getTime() === today.getTime()) {
      classes.push('aiab-datepicker-date--today');
    }

    if (this.isDateSelected(date)) {
      classes.push('aiab-datepicker-date--selected');
    }

    if (this.state!.focusedDate && date.getTime() === this.state!.focusedDate.getTime()) {
      classes.push('aiab-datepicker-date--focused');
    }

    if (this.options!.range && this.state!.rangeStart && this.state!.rangeEnd) {
      if (date >= this.state!.rangeStart && date <= this.state!.rangeEnd) {
        classes.push('aiab-datepicker-date--in-range');
      }
    }

    return classes.join(' ');
  }

  private isDateSelected(date: Date): boolean {
    return this.state!.selectedDates.some(
      (d: Date) =>
        d.getFullYear() === date.getFullYear() &&
        d.getMonth() === date.getMonth() &&
        d.getDate() === date.getDate(),
    );
  }

  private isDateDisabled(date: Date): boolean {
    // Check min/max dates
    if (this.options!.minDate && date < this.options!.minDate) return true;
    if (this.options!.maxDate && date > this.options!.maxDate) return true;

    // Check disabled days of week
    if (this.options!.disabledDays.includes(date.getDay())) return true;

    // Check specific disabled dates
    return this.options!.disabledDates.some((d: Date | string) => {
      const disabled = new Date(d);
      return (
        disabled.getFullYear() === date.getFullYear() &&
        disabled.getMonth() === date.getMonth() &&
        disabled.getDate() === date.getDate()
      );
    });
  }

  // --- Localization helpers ---

  private getMonthNames(): string[] {
    const formatter = new Intl.DateTimeFormat(this.options!.locale, { month: 'short' });
    return Array.from({ length: 12 }, (_, i) => formatter.format(new Date(2000, i, 1)));
  }

  private getWeekDays(): string[] {
    const formatter = new Intl.DateTimeFormat(this.options!.locale, { weekday: 'short' });
    const days: string[] = [];
    for (let i = 0; i < 7; i++) {
      const day = new Date(2000, 0, 2 + ((i + this.options!.weekStart) % 7));
      days.push(formatter.format(day));
    }
    return days;
  }

  // --- Update methods ---

  private updateHeader(): void {
    const formatter = new Intl.DateTimeFormat(this.options!.locale, {
      month: 'long',
      year: 'numeric',
    });
    const parts = formatter.formatToParts(this.state!.viewDate);

    const monthPart = parts.find((p) => p.type === 'month');
    const yearPart = parts.find((p) => p.type === 'year');

    this.monthText.textContent = monthPart ? monthPart.value : '';
    this.yearText.textContent = yearPart ? yearPart.value : '';
  }

  private updatePosition(): void {
    if (!this.state!.isOpen || this.options!.inline) return;

    const rect = this.element!.getBoundingClientRect();
    const calendarHeight = this.calendar.offsetHeight;

    // Reset classes
    this.calendar.classList.remove(
      'aiab-datepicker-calendar--top',
      'aiab-datepicker-calendar--bottom',
    );

    // Auto position
    if (this.options!.position === 'auto') {
      const spaceBelow = window.innerHeight - rect.bottom;
      const spaceAbove = rect.top;

      if (spaceBelow < calendarHeight && spaceAbove > spaceBelow) {
        this.calendar.classList.add('aiab-datepicker-calendar--top');
      } else {
        this.calendar.classList.add('aiab-datepicker-calendar--bottom');
      }
    } else {
      this.calendar.classList.add(`aiab-datepicker-calendar--${this.options!.position}`);
    }
  }

  private updateInput(): void {
    const formatter = new Intl.DateTimeFormat(this.options!.locale, {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
    });

    if (this.options!.multiple) {
      const dates = this.state!.selectedDates.map((d: Date) => formatter.format(d));
      this.element!.value = dates.join(', ');
    } else if (this.options!.range && this.state!.rangeStart && this.state!.rangeEnd) {
      this.element!.value =
        `${formatter.format(this.state!.rangeStart)} - ${formatter.format(this.state!.rangeEnd)}`;
    } else if (this.state!.selectedDates.length > 0) {
      this.element!.value = formatter.format(this.state!.selectedDates[0]);
    } else {
      this.element!.value = '';
    }
  }

  // --- Navigation methods ---

  private navigate(direction: number): void {
    switch (this.state!.viewMode) {
      case 'days':
        this.state!.viewDate.setMonth(this.state!.viewDate.getMonth() + direction);
        break;
      case 'months':
        this.state!.viewDate.setFullYear(this.state!.viewDate.getFullYear() + direction);
        break;
      case 'years':
        this.state!.viewDate.setFullYear(this.state!.viewDate.getFullYear() + direction * 10);
        break;
    }
    this.render();
  }

  public setViewMode(mode: ViewMode): void {
    this.state!.viewMode = mode;
    this.render();
  }

  // --- Selection methods ---

  public selectDate(date: Date): void {
    if (this.isDateDisabled(date)) return;

    if (this.options!.multiple) {
      const index = this.state!.selectedDates.findIndex(
        (d: Date) =>
          d.getFullYear() === date.getFullYear() &&
          d.getMonth() === date.getMonth() &&
          d.getDate() === date.getDate(),
      );

      if (index > -1) {
        this.state!.selectedDates.splice(index, 1);
      } else {
        this.state!.selectedDates.push(date);
      }
    } else if (this.options!.range) {
      if (!this.state!.rangeStart || this.state!.rangeEnd) {
        this.state!.rangeStart = date;
        this.state!.rangeEnd = null;
      } else {
        if (date < this.state!.rangeStart) {
          this.state!.rangeEnd = this.state!.rangeStart;
          this.state!.rangeStart = date;
        } else {
          this.state!.rangeEnd = date;
        }
      }
      this.state!.selectedDates = [this.state!.rangeStart, this.state!.rangeEnd].filter(
        Boolean,
      ) as Date[];
    } else {
      this.state!.selectedDates = [date];
    }

    this.state!.viewDate = new Date(date);
    this.updateInput();
    this.render();

    // Fire callbacks
    if (this.options!.onSelect) {
      this.options!.onSelect(date, this);
    }

    if (this.options!.onChange) {
      this.options!.onChange(this.getValue(), this);
    }

    // Auto close
    if (this.options!.autoClose && !this.options!.multiple && !this.options!.range) {
      this.close();
    }
  }

  public selectToday(): void {
    this.selectDate(new Date());
  }

  public clear(): void {
    this.state!.selectedDates = [];
    this.state!.rangeStart = null;
    this.state!.rangeEnd = null;
    this.updateInput();
    this.render();

    if (this.options!.onChange) {
      this.options!.onChange(null, this);
    }
  }

  // --- Open/Close methods ---

  public open(): void {
    if (this.state!.isOpen) return;

    this.state!.isOpen = true;
    this.calendar.classList.add('aiab-datepicker-calendar--open');
    this.calendar.setAttribute('aria-hidden', 'false');

    this.updatePosition();

    if (this.options!.onOpen) {
      this.options!.onOpen(this);
    }
  }

  public close(): void {
    if (!this.state!.isOpen || this.options!.inline) return;

    this.state!.isOpen = false;
    this.calendar.classList.remove('aiab-datepicker-calendar--open');
    this.calendar.setAttribute('aria-hidden', 'true');

    if (this.options!.onClose) {
      this.options!.onClose(this);
    }
  }

  public toggle(): void {
    if (this.state!.isOpen) {
      this.close();
    } else {
      this.open();
    }
  }

  // --- Parse initial value ---

  private parseInitialValue(): void {
    try {
      const date = new Date(this.element!.value);
      if (!Number.isNaN(date.getTime())) {
        this.state!.selectedDates = [date];
        this.state!.viewDate = new Date(date);
      }
    } catch (_e) {
      // Invalid date, ignore
    }
  }

  // --- Public API methods ---

  public getValue(): Date | Date[] | DatePickerEnhancedRangeValue | null {
    if (this.options!.multiple) {
      return this.state!.selectedDates;
    }
    if (this.options!.range) {
      return {
        start: this.state!.rangeStart,
        end: this.state!.rangeEnd,
      };
    }
    return this.state!.selectedDates[0] || null;
  }

  public setValue(value: Date | string | Date[] | SetRangeValue | null): void {
    if (this.options!.multiple && Array.isArray(value)) {
      this.state!.selectedDates = value.map((v: Date | string) => new Date(v));
    } else if (
      this.options!.range &&
      value !== null &&
      typeof value === 'object' &&
      !Array.isArray(value) &&
      !(value instanceof Date) &&
      'start' in value &&
      'end' in value
    ) {
      this.state!.rangeStart = new Date(value.start);
      this.state!.rangeEnd = new Date(value.end);
      this.state!.selectedDates = [this.state!.rangeStart, this.state!.rangeEnd];
    } else if (value) {
      this.state!.selectedDates = [new Date(value as Date | string)];
    } else {
      this.clear();
    }

    this.updateInput();
    this.render();
  }

  public setOptions(options: DatePickerEnhancedOptions): void {
    this.options = { ...this.options!, ...options } as ResolvedOptions;
    this.render();
  }

  /**
   * Comprehensive destroy method with full cleanup.
   * Removes all event listeners, timers, DOM elements, and restores the input.
   */
  public destroy(): void {
    // Close if open
    this.close();

    // Remove all event listeners
    this.handlers.forEach((handlerList: HandlerEntry[], element: EventTarget) => {
      for (const { event, handler, options } of handlerList) {
        element.removeEventListener(event, handler, options);
      }
    });
    this.handlers.clear();

    // Remove dynamic event listeners
    this.dynamicHandlers.forEach((entry: DynamicHandlerEntry, element: Element) => {
      element.removeEventListener(entry.event, entry.handler);
    });
    this.dynamicHandlers.clear();

    // Clear all timers
    this.timers.forEach((timer) => clearTimeout(timer));
    this.timers.clear();

    // Remove all created DOM elements
    this.createdElements.forEach((element: HTMLElement) => {
      if (element.parentNode) {
        element.parentNode.removeChild(element);
      }
    });
    this.createdElements.clear();

    // Restore original input
    if (this.wrapper?.parentNode) {
      this.wrapper.parentNode.insertBefore(this.element!, this.wrapper);
      this.wrapper.parentNode.removeChild(this.wrapper);
    }

    // Remove input modifications
    this.element!.classList.remove('aiab-datepicker-input');
    this.element!.removeAttribute('readonly');

    // Clear all references
    this.element = null;
    this.wrapper = null!;
    this.calendar = null!;
    this.body = null!;
    this.prevBtn = null!;
    this.nextBtn = null!;
    this.monthText = null!;
    this.yearText = null!;
    this.todayBtn = null!;
    this.clearBtn = null!;

    // Clear state
    this.state = null;
    this.options = null;
  }
}

// Extend Window interface for global assignments
declare global {
  interface Window {
    DatePickerEnhanced: typeof DatePickerEnhanced;
  }
}

// Register with component registry if available
if (window.AmphibiousRegistry) {
  // biome-ignore lint/suspicious/noExplicitAny: constructor type variance for registry
  window.AmphibiousRegistry.registerComponent('aiab-datepicker', DatePickerEnhanced as any, {
    selector: '[data-datepicker]',
    autoInit: true,
  });
}

// Export
window.DatePickerEnhanced = DatePickerEnhanced;
export default DatePickerEnhanced;
export { DatePickerEnhanced };
