...
(($, window, document, undefined) => {
  ...
  var /** Used later to scroll page to the top */
    $body = $('html, body'),
    ...
    utility = {
      ...
      htmlDoc: (html) => {
        var parent,
          elems = $(),
          matchTag = /<(\/?)(html|head|body|title|base|meta)(\s+[^>]*)?>/gi,
          ...
          htmlParsed = html.replace(matchTag, (tag, slash, name, attrs) => {
            ...
            return (
              '<' + slash + 'div' + (slash ? '' : " id='" + prefix + (elems.length - 1) + "'") + '>'
            );
          });
      ...
      getContentById: (id, $html) => {
        ...
        var $insideElem = $html.find(id),
          updatedContainer = $insideElem.length
            ? $.trim($insideElem.html())
            : $html.filter(id).html(),
    ...
    SmoothState = (element, options) => {
      var /** Container element smoothState is run on */
        $container = $(element),
        ...
        load = (url, isPopped) => {
          ...
          var /** Used to check if the onProgress function has been run */
