...
(
  ($) => {
    ...
    ts.addParser({
      id: 'text',
      is: (s) => true,
      format: (s) => $.trim(s.toLocaleLowerCase()),
      type: 'text',
    });
    ...
    ts.addParser({
      ...
      format: (s) => $.tablesorter.formatFloat(s),
      type: 'numeric',
    });
    ...
    ts.addParser({
      id: 'currency',
      is: (s) => /^[£$€?.]/.test(s),
      format: (s) => $.tablesorter.formatFloat(s.replace(new RegExp(/[£$€]/g), '')),
      type: 'numeric',
    });

    ts.addParser({
      id: 'ipAddress',
      is: (s) => /^\d{2,3}[\.]\d{2,3}[\.]\d{2,3}[\.]\d{2,3}$/.test(s),
      format: (s) => {
        var a = s.split('.'),
          r = '',
          l = a.length;
    ...
    ts.addParser({
      id: 'url',
      is: (s) => /^(https?|ftp|file):\/\/$/.test(s),
      format: (s) => jQuery.trim(s.replace(new RegExp(/(https?|ftp|file):\/\//), '')),
      type: 'text',
    });

    ts.addParser({
      id: 'isoDate',
      is: (s) => /^\d{4}[\/-]\d{1,2}[\/-]\d{1,2}$/.test(s),
      format: (s) =>
    ...
    ts.addParser({
      ...
      format: (s) => $.tablesorter.formatFloat(new Date(s).getTime()),
      type: 'numeric',
    });

    ts.addParser({
      id: 'shortDate',
      is: (s) => /\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/.test(s),
      format: (s, table) => {
        var c = table.config;
        s = s.replace(/\-/g, '/');
        if (c.dateFormat == 'us') {
    ...
    ts.addParser({
      id: 'time',
      is: (s) => /^(([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(am|pm)))$/.test(s),
      format: (s) => $.tablesorter.formatFloat(new Date('2000/01/01 ' + s).getTime()),
      type: 'numeric',
    });
    ts.addParser({
      id: 'metadata',
